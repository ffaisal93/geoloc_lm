...
def transformers_model_name_to_family(model_name: str) -> str:
    ...
    if model_name.startswith("bert"):
        ...
        return "gpt2"
    elif model_name.startswith("sberbank-ai/mGPT"):
        return "gpt2"
...
def get_layer_regex(model_name: str) -> t.Optional[t.List[str]]:
    ...
    if family == "gpt2":
        ...
        layer_types = [
            "h.([0-9]|[0-9][0-9]).mlp.dense_h_to_4h",
            "h.([0-9]|[0-9][0-9]).mlp.dense_4h_to_h",
            "h.([0-9]|[0-9][0-9]).input_layernorm",
            "h.([0-9]|[0-9][0-9]).self_attention.query_key_value",
            "h.([0-9]|[0-9][0-9]).self_attention.dense",
            "h.([0-9]|[0-9][0-9]).post_attention_layernorm",
        ]
