...
dir=${1:-"/projects/antonis/fahim/ner_linking/data/geoloc/BD-bn/bloom/data"}
outdir=${2:-"/projects/antonis/fahim/ner_linking/data/geoloc/BD-bn/bloom"}
mode=${3:-"generate"}
label='KR-ko'
...
do
	if [ "$file" != "${dir}/**DS_Store**" ] && [ "$file" != "${dir}/**._**" ]; then
		...
		echo ../tr_output/${label}_${base}.err
		echo ${outdir}
		sbatch -o ../tr_output/${label}_${base}.out -e ../tr_output/${label}_${base}.err geoloc_dg_h.slurm $file $base $outdir $mode
	fi
done
